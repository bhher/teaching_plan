...
<body>
	...
	function previewImage(input){
		...
		// 사용자가 파일을 선택했는지, 그리고 그 파일이 존재하는지 확인
		if (input.files && input.files[0]) {
            var reader = new FileReader();
            // 파일을 읽어줄 독서기 (reader) 객체 생성
            
            // 파일 읽기가 완료되면 실행될 동작을 정의
            reader.onload = function(e) {
            	// e.target.result: 읽어온 이미지 데이터
                previewImage.src = e.target.result;  // <img>태그의 주소 저장
                previewContainer.style.display = 'block';
                // 숨겨져 있던 컨테이너를 화면에 표시
            };
            
            // 실제 파일을 읽어서 '데이터 URL' 형태로 변환 시작
            reader.readAsDataURL(input.files[0]);
        } else {
        	// 파일을 선택 취소하거나 비어있으면 미리보기 영역을 숨김
